      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-brand-50 p-6 rounded-xl border border-brand-100 shadow-sm">
          ...
          <div className="mt-6 pt-6 border-t border-brand-200 text-sm text-brand-800 space-y-3">
            ...
            {quote.takedownsIncluded ? (
              <div className="flex justify-between items-center text-green-700">
                <span className="font-medium">Takedowns ({takedownsCount}):</span>
                <span className="text-xs font-bold bg-green-100 px-2 py-1 rounded uppercase">Included</span>
              </div>
            ) : (
              takedownsCost > 0 && (
            ...
            {discount > 0 && (
              <div className="flex justify-between items-center text-red-600 bg-red-50 p-2 rounded -mx-2">
                <span className="font-bold uppercase text-xs">Discount applied:</span>
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
            <p className="text-xs text-gray-500 uppercase font-bold tracking-widest mb-3">Items Breakdown</p>
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {quote.blinds.map((blind, i) => (
                <div key={blind.id} className="flex justify-between text-xs">
                  <span className="text-gray-600 truncate mr-2">{i + 1}. {blind.room} — {blind.type}</span>
                  <span className="font-bold text-gray-800 whitespace-nowrap">${blind.price.toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
